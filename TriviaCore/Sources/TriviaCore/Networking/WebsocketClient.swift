...
@Reducer
public struct WebsocketClient: LoggingContext, Sendable {
...
final class WebsocketDelegate: NSObject, URLSessionWebSocketDelegate {
  let onClose: @Sendable (_ closure: (URLSessionWebSocketTask.CloseCode)) -> Void
  
  init(onClose: @Sendable @escaping (_ closure: (URLSessionWebSocketTask.CloseCode)) -> Void) {
...
extension WebsocketClientActor: DependencyKey {
  static let liveValue: @Sendable () -> WebsocketClientActor = {
  ...
  static var testValue: @Sendable () -> WebsocketClientActor = {
    .init()
  }
}

extension DependencyValues {
  var websocket: @Sendable () -> WebsocketClientActor {
